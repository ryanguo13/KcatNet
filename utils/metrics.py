...
from sklearn.metrics import average_precision_score, roc_auc_score, f1_score, recall_score, precision_score, accuracy_score,mean_squared_error, r2_score
import numpy as np
...
def get_rmse(y,f):
    rmse = np.sqrt(mean_squared_error(y, f)) #sqrt(((y - f)**2).mean(axis=0))
    return rmse
...
def get_pearson(y,p):
    Y = np.array(y)
    P = np.array(p)
    rp, p_value = pearsonr(Y,P)
    return rp

def get_spearman(y,p):
    sp = stats.spearmanr(y,p)[0]
    return sp
...
def evaluate_reg(Y, F):
    ...
    res_mse=float(get_mse(Y, F))
    res_rmse= float(get_rmse(Y, F))
    res_r2_score = float(get_r2(Y, F))
    res_pearson= float(get_pearson(Y, F))
    res_spearman= float(get_spearman(Y, F))

    return { 
        'mse': res_mse,
        'rmse':res_rmse,
        'pearson': res_pearson,
        'spearman': res_spearman,
        "r2_score":res_r2_score,
