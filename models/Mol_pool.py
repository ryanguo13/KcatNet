...
class MotifPool(torch.nn.Module):
    ...
    def forward(self, x, mol_batch): #x(321,200)
        H = self.heads
        C = self.hidden_dim

        atom_feature = x.view(-1, H, C) #(321,5,40)
        score = torch.cat([mlp(atom_feature[:, i]) for i, mlp in enumerate(self.score_proj)], dim=-1)#(321,5)
        alpha = softmax(score, mol_batch) #(321,5)
        drug_feat = atom_feature.view(-1, H, C) * alpha.view(-1, H, 1)#(321,5,40)
        drug_feat = global_add_pool(drug_feat.view(-1, H * C), mol_batch)  #(B,200)
